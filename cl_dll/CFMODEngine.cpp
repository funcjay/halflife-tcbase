...
void CFMODEngine::Initialize()
{
	...
	{
		gEngfuncs.Con_Printf( "[FMOD] Failed to create the system! (%s)\n", FMOD_ErrorString( m_pResult ) );
		m_iSystemState = 2;
		return;
	}
...
void CFMODEngine::InitializeHUD()
{
	HOOK_MESSAGE( FMOD );
}
...
void CFMODEngine::PlayMusic( const char *_cMusic, const float _flVol, const bool _bLooped )
{
	...
	char cFullPath[64];
	sprintf( cFullPath, "%s/sound/music/%s", gEngfuncs.pfnGetGameDirectory(), _cMusic );
#ifdef WIN32
	if ( _access( cFullPath, 4 ) != 0 )
#else
	if ( _access( cFullPath, R_OK ) != 0 )
#endif
	{
...
bool CFMODEngine::InitializeCheckFMODVersion()
{
...
bool CFMODEngine::InitializeGetFunctionsPointers()
{
#ifdef WIN32
...
bool CFMODEngine::InitializeLoadLibrary()
{
